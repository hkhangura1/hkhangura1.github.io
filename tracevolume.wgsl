...
fn traceScene(uv: vec2i, p: vec3f, d: vec3f) {
  ...
  if (hits.x >= 0) { 
    ...
    /*var curT = hits.x + EPSILON;
    let voxelSize = vec3f(volInfo.sizes.xyz);
    var currentInt = 0.5f;
    //let volOrigin = vec3f(0, 0, 0);
    
    //ray marching
    while (curT < hits.y) {
      let hitPt = p + d * curT;
      let eps = 1e-5;
      //let local = floor((hitPt + halfsize.xyz) / voxelSize.xyz - eps);
      let local = clamp(
        vec3<i32>(floor((hitPt + halfsize.xyz) / voxelSize.xyz)),
        vec3<i32>(0),
        vec3<i32>(volInfo.dims.xyz) - vec3<i32>(1)
      );


      let dims = vec3<i32>(volInfo.dims.xyz);
      var voxelIndex = local.z * dims.x * dims.y + local.y * dims.x + local.x; //k

      //voxelIndex = 1001;
      let voxelValue = volData[u32(voxelIndex)];
      //currentInt = max(currentInt, voxelValue);
      currentInt = currentInt + voxelValue;
      //hitValues = getNextHitValue(halfsize.z, halfsize.xy, p.z, d.z, p.xy, d.xy, hitValues); 
      //let ix = voxelIndex % volInfo.dims.x;
      //let iy = (voxelIndex / volInfo.dims.x) % volInfo.dims.y;
      //let iz = (voxelIndex / volInfo.dims.x) / volInfo.dims.y;
      //let voxelCorners = (vec3f(x * volInfo.sizes.x),(y * volInfo.sizes.y),(z * volInfo.sizes.z));
      //let minCorner = vec3f(-halfsize.x + f32(ix) * voxelSize.x, -halfsize.y + f32(iy) * voxelSize.y, -halfsize.z + f32(iz) * voxelSize.z);
      //let minCorner = local * voxelSize.xyz - halfsize.xyz;
      let minCorner = vec3<f32>(local) * voxelSize - halfsize.xyz;
      let maxCorner = minCorner + voxelSize;
    
      curT = getNextHitValue(hits.x, curT, minCorner.z, minCorner.xy, maxCorner.xy, p.z, d.z, p.xy, d.xy);
      curT = getNextHitValue(hits.x, curT, maxCorner.z, minCorner.xy, maxCorner.xy, p.z, d.z, p.xy, d.xy);

      curT = getNextHitValue(hits.x, curT, minCorner.y, minCorner.xz, maxCorner.xz, p.y, d.y, p.xz, d.xz);
      curT = getNextHitValue(hits.x, curT, maxCorner.y, minCorner.xz, maxCorner.xz, p.y, d.y, p.xz, d.xz);
      
      curT = getNextHitValue(hits.x, curT, minCorner.x, minCorner.yz, maxCorner.yz, p.x, d.x, p.yz, d.yz);
      curT = getNextHitValue(hits.x, curT, maxCorner.x, minCorner.yz, maxCorner.yz, p.x, d.x, p.yz, d.yz);
    
      curT = curT + 0.00001; // add a little offset to make it is it enters the next volxel
    }
    
    
    //currentInt = currentInt / 4090.f;
    currentInt = currentInt / f32(volInfo.dims.x * volInfo.dims.y * volInfo.dims.z);
    //color = vec4f(currentInt, currentInt, currentInt, 1.f);
    color = vec4f(volData[0], volData[0], volData[0], 1);
    //color = vec4f(voxelValue.dims, voxelValue.dims, voxelValue.dims, 1.0);
    */

   /* if (hits.x < 0.0) {
      color = vec4f(1, 0, 0, 1); // red = no intersection
      textureStore(outTexture, uv, color);
      return;
    }*/

    var curT = hits.x + EPSILON;
    let voxelSize = volInfo.sizes.xyz;
    let dims = vec3<i32>(volInfo.dims.xyz);

    var currentInt: f32 = 0.0;
    var steps = 0;

    while (curT < hits.y && steps < 512) {
      steps++;

      let hitPt = p + d * curT;

      let local = clamp(
        vec3<i32>(floor((hitPt) / voxelSize)),
        vec3<i32>(0),
        dims - vec3<i32>(1)
      );

      let voxelIndex =
          local.z * dims.x * dims.y +
          local.y * dims.x +
          local.x;
      ...
      nextT = getNextHitValue(curT, nextT, minCorner.z, minCorner.xy, maxCorner.xy, p.z, d.z, p.xy, d.xy);
      nextT = getNextHitValue(curT, nextT, maxCorner.z, minCorner.xy, maxCorner.xy, p.z, d.z, p.xy, d.xy);

      nextT = getNextHitValue(curT, nextT, minCorner.y, minCorner.xz, maxCorner.xz, p.y, d.y, p.xz, d.xz);
      nextT = getNextHitValue(curT, nextT, maxCorner.y, minCorner.xz, maxCorner.xz, p.y, d.y, p.xz, d.xz);

      nextT = getNextHitValue(curT, nextT, minCorner.x, minCorner.yz, maxCorner.yz, p.x, d.x, p.yz, d.yz);
      nextT = getNextHitValue(curT, nextT, maxCorner.x, minCorner.yz, maxCorner.yz, p.x, d.x, p.yz, d.yz);

      curT = nextT + EPSILON;
    }

    // FINAL COLOR
    currentInt = currentInt / 4095.0;
    color = vec4f(currentInt, currentInt, currentInt, 1.0);

    // normalize intensity
    //currentInt = currentInt / 4095.0;
    //color = vec4f(voxelValue, voxelValue, voxelValue, 1.0);
    //return;
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  let texDim = vec2i(textureDimensions(outTexture));
  if (uv.x < texDim.x && uv.y < texDim.y) {
    let psize = vec2f(2, 2) * tan(cameraPose.focal.xy) / cameraPose.res.xy;
    //let psizeFocal = vec2f(2, 2) * ((cameraPose.res.xy / 2)) / cameraPose.focal.xy;
    var spt = vec3f(0, 0, 0);
    let dirx = (f32(uv.x) + 0.5) * psize.x - (cameraPose.res.x * psize.x) / 2;
    let diry = (f32(uv.y) + 0.5) * psize.y - (cameraPose.res.y * psize.y) / 2;
    var rdir = normalize(vec3f(dirx, diry, 1.0));
    spt = transformPt(spt);
    rdir = transformDir(rdir);
    //var hitInfo = rayVolumeIntersection(spt, rdir);
    //assignColor(uv);
    traceScene(uv, spt, rdir);
  }
}
